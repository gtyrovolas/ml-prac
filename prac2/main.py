...
class NBC:
    ...
    def predict(self, X):
        ...
        for x_i in X:
            ...
            for p_c, (c, theta_c) in list(
                    zip(self.p.values(), self.theta.items())):
                log_prob = np.log(p_c)
...
def run(X, y):
    ...
    NBC_acc_list = [
        testModel(nbc, Xtrains[k], ytrains[k], Xtest, ytest)
        for k in range(10)
    ]
    LR_acc_list = [
        testModel(log_reg, Xtrains[k], ytrains[k], Xtest, ytest)
        for k in range(10)
    ]
...
def mkLineGraph(NBC_err, LR_err):
    ...
    plt.plot(x, NBC_err, color='skyblue', linewidth=1.5,
             label="Naive Bayes Classifier Error")
    plt.plot(x, LR_err, color='red', linewidth=1.5,
             label="Logistic Regression Error")
...
def main():
    ...
    # Handin 1: As C is the inverse of regularisation strength I would choose
    # C = 10 for lambda = 0.1
